                       ...
                       const bool use_gpu_detect)
{
  ...
  this->use_gpu_detect = use_gpu_detect;
                          ...
                          int &newPtscount)
{
  ...
  if(use_gpu_detect)
  {
    //cout << "\033[1;31m !!! use gpu redetect !!!! \033[0m" << endl;
    cv::cuda::GpuMat cur_gpu_img(img);
    cv::cuda::GpuMat gpu_Pts;
    cv::cuda::GpuMat gpu_mask(mask);

    cv::Ptr<cv::cuda::CornersDetector> detector = cv::cuda::createGoodFeaturesToTrackDetector(cur_gpu_img.type(), gftt_num*2,
                                                                                              gftt_ql, gftt_dis);
    //tic_toc_ros t_d;
    detector->detect(cur_gpu_img, gpu_Pts, gpu_mask);
    //t_d.toc("gpu detect cost");
    if(!gpu_Pts.empty())
    {
      features = cv::Mat_<cv::Point2f>(cv::Mat(gpu_Pts));
    }

  }
  else
  {
    cv::goodFeaturesToTrack(img,features, gftt_num*2, gftt_ql, gftt_dis, mask);
...
void FeatureDEM::detect(const cv::Mat& img, vector<cv::Point2f>& newPts)
{
  ...
  if(use_gpu_detect)
  {
    ...
    cv::Ptr<cv::cuda::CornersDetector> detector = cv::cuda::createGoodFeaturesToTrackDetector(cur_gpu_img.type(), gftt_num*2,
                                                                                              gftt_ql, gftt_dis);
    detector->detect(cur_gpu_img, gpu_Pts);
    if(!gpu_Pts.empty())
    {
      features = cv::Mat_<cv::Point2f>(cv::Mat(gpu_Pts));
    }
