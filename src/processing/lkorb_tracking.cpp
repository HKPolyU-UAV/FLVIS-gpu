...
LKORBTracking::LKORBTracking(int width_in,int height_in, bool use_gpu_flow)
{
  this->width=width_in;
  this->height=height_in;
  this->use_gpu_flow = use_gpu_flow;
                             ...
                             vector<cv::Point2f>& outlier)
{
  ...
  vector<float>         err;
  cv::cuda::GpuMat from_gpu_img0(from.img0);
  cv::cuda::GpuMat to_gpu_img0(to.img0);
  cv::cuda::GpuMat from_gpu_pts(from_p2d_plane);
  cv::cuda::GpuMat to_gpu_pts(tracked_p2d_plane);
  ...
  if(use_gpu_flow)
  {
    if(use_guess){//project 3d lms to 2d using guess
      ...
      cv::Ptr<cv::cuda::SparsePyrLKOpticalFlow> d_pyrLK_sparse = cv::cuda::SparsePyrLKOpticalFlow::create(
            cv::Size(31,31), 10, 30, true);
      d_pyrLK_sparse->calc(from_gpu_img0, to_gpu_img0, from_gpu_pts, to_gpu_pts, gpu_status);
      vector<cv::Point2f> tmp_to_pts(to_gpu_pts.cols);
      to_gpu_pts.download(tmp_to_pts);
      tracked_p2d_plane = tmp_to_pts;

      vector<uchar> tmp_status(gpu_status.cols);
      gpu_status.download(tmp_status);
      mask_tracked = tmp_status;
    }
    else
    {
      cv::Ptr<cv::cuda::SparsePyrLKOpticalFlow> d_pyrLK_sparse = cv::cuda::SparsePyrLKOpticalFlow::create(
  ...
  int of_inlier_cnt=0;
  //cout << "\033[1;32m !!! STEP 1!!!! \033[0m" << endl;
