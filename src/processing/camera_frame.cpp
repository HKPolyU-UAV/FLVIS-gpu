...
void CameraFrame::setGpu(bool use_gpu){
                                            ...
                                            float range)
{
  ...
  if(use_gpu)
  {
    cv::cuda::GpuMat gpu_img0(this->img0);
    cv::cuda::GpuMat gpu_img1(this->img1);
    cv::cuda::GpuMat gpu_pts0(pt2d_0_plane);
    cv::cuda::GpuMat gpu_pts1(pt2d_1_plane);
    cv::cuda::GpuMat gpu_status;
    //cout << "\033[1;32m !!! use gpu recoverDepth!!!! \033[0m" << endl;
    cv::Ptr<cv::cuda::SparsePyrLKOpticalFlow> d_pyrLK_sparse = cv::cuda::SparsePyrLKOpticalFlow::create(
          cv::Size(31,31), 10, 30, true);
    d_pyrLK_sparse->calc(gpu_img0, gpu_img1, gpu_pts0, gpu_pts1, gpu_status);
    vector<cv::Point2f> tmp_pts(gpu_pts1.cols);
    gpu_pts1.download(tmp_pts);
    pt2d_1_plane = tmp_pts;
