...
private:
    ...
    void frame_callback(const flvis::KeyFrameConstPtr& msg)
    {
        ...
        case STEREO_RECT:
        {
            ...
            if(use_gpu)
            {
              cv::cuda::GpuMat gpu_img0(img0_unpack);
              cv::cuda::GpuMat gpu_img1(img1_unpack);
              cv::cuda::GpuMat gpu_pts0(lm_img0);
              cv::cuda::GpuMat gpu_pts1(lm_img1);
    ...
    virtual void onInit()
    {
        ...
        use_gpu = getBoolVariableFromYaml(configFilePath, "use_gpu");
